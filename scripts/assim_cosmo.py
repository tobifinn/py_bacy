...
from py_bacy.intf_pytassim import obs_op, cosmo, io, utils
...
parser = argparse.ArgumentParser(
    description='This script is used to assimilate T2m into COSMO files and '
                'to generated analysis files for COSMO',
    prog='PyTassim assimilation for COSMO'
)
parser.add_argument(
...
parser.add_argument(
    '--bg_files', type=str, default='*_ana',
    help='Regex suffix for background data files (default=`*_ana`)'
)
parser.add_argument(
...
parser.add_argument(
    '--loc_radius', type=float, default=(50000, 0.3), nargs='+',
    help='Localization radii, which are used to initialize Gaspari-Cohn '
         'localization (horizontal in meters, vertical in ln(hPa)), '
         'default=(50000, 0.3)'
)
parser.add_argument(
...
def init_assim(loc_radius, executor, inf_factor=1.1, chunksize=1000):
    localization = GaspariCohn(
        np.array(loc_radius), dist_func=cosmo.distance_func
    )
...
def assimilate_cosmo(
        run_dir, util_dir, file_path_obs, start_time, end_time,
        ensemble_members, bg_files, fg_files, assim_vars, assim_obj
):
    ds_bg = cosmo.load_background(run_dir, bg_files, ensemble_members)
    ds_const = cosmo.load_constant_data(util_dir)
    state_bg = cosmo.get_bg_array(ds_bg, ds_const, end_time, assim_vars,)
    ...
    ds_ana = postprocess_cosmo(state_analysis, ds_bg)
    ds_ana = cosmo.correct_vars(ds_ana, ds_bg)
    utils.info_assimilation(ds_ana, ds_bg, assim_vars, run_dir, 'cosmo')
    cosmo.write_analysis(ds_ana, run_dir, bg_files, end_time, ensemble_members,
                         assim_vars)
...
if __name__ == '__main__':
    ...
    assimilate_cosmo(
        ...
        args.end_time, args.ensemble_members, args.bg_files, args.fg_files,
        args.assim_vars, assim_obj
    )
